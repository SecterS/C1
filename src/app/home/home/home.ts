...
export class Home implements AfterViewInit, OnInit {
  ...
  // Профиль пользователя (заглушка + localStorage)
  currentUser = {
    name: 'Suganov Peter',
    role: 'Pro Plan',
  ...
  ngOnInit(): void {
    if (isPlatformBrowser(this.platformId)) {
      // 1. Загружаем пользователя (если есть логин)
  ...
  getTasksForDay(day: CalendarDay): Task[] {
    let dayTasks = this.tasks.filter(task => {
      ...
      // Сравниваем даты. Выполненные задачи ТОЖЕ показываем (но они будут зачеркнуты стилями)
  ...
  private generateCalendar(): void {
    ...
    // Определяем день недели (Пн=0, Вс=6)
    ...
    // 3. Дни следующего месяца (Добиваем ровно до 42 ячеек - 6 строк)
  ...
  onTaskSaved(taskData: any): void {
    if (taskData.id) {
      this.taskService.updateTask(taskData).subscribe(updated => {
